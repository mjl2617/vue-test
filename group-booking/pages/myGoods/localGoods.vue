		...
		<el-table ref="multipleTable" :data="tableData" border style="width: 100%"
			...
			<el-table-column prop="source" label="来源">
				<template slot-scope="scope">
					...
					<span v-if="scope.row.source == 20">千城</span>
				</template>
			</el-table-column>
			<el-table-column prop="source" label="状态">
				<template slot-scope="scope">
					<div v-if="scope.row.state== 1">
						<span >上架</span>
						<span v-if="scope.row.skuList[0].stock == 0">/已售罄</span>
					</div>
			...
			<el-table-column label="操作">
				<template slot-scope="scope">
					...
					<div @click="handleClick(scope.row,3)" style="color: #409EFF;cursor: pointer;">设置商品信息</div>
				...
				<el-form style="display: flex;flex-direction: row;" ref="form" :model="form" label-width="80px">
					...
					<el-form-item label="分类筛选">
						<el-select v-model="form3.categoryId " placeholder="请选择">
							<el-option v-for="(item3,index) in categoryimportList " :key="index" :label="item3.name"
								:value="item3.id">
							</el-option>
						</el-select>
					</el-form-item>
		...
		methods: {
			...
			async submit() {
				...
				let {
					...
					url: "/shop/Goods/QCWPGoodsDetials",
					method: "POST",
					data: {
						id: this.goodsId,
				...
				if (errorCode == '0') {
					this.$router.push('/myGoods/information?goodsId='+errorMessage)
				}
			...
			async getData() {
				let {
					...
					data: {
						...
						"subCategoryId": this.form.categoryId,
				...
				this.tableData.map((item) => {
					...
					let picturelist = item.picture.split(',')
					item.picture  = picturelist[0]
			...
			handleClick(row, type) {
				if (type == 0) { //设置分类
					...
					this.$router.push('/myGoods/information?goodsId='+row.goodsId)
