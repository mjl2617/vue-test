<template>
	<div class="page">
		...
		<el-table :data="tableData" border style="width: 100%">
			...
			<el-table-column prop="allUser" label="总用户数">
			</el-table-column>
			
			
			<el-table-column prop="activeUser" label="今日活跃数">
			</el-table-column>
			<el-table-column prop="num" label="日活率(%)">
			</el-table-column>
		...
		methods: {
			async getData() {
				let {
					...
					url: "base/Admin/GetTeamActivity",
					method: "POST",
					data: {
						"userId": this.id,
				...
				for(let i =0;i<list.length;i++){
					if(list[i].allUser==0){
						...
						list[i].num = (list[i].activeUser)/(list[i].allUser)*100
					}
				}
				this.tableData = list;
